...
// SendPrivateRawTransaction will add the signed raw transaction to the transaction pool.
// The sender is responsible for signing the transaction and using the correct nonce.
...
// GetEncryptedHash return the encrypted hash from crux
// The sender is responsible for signing the transaction and using the correct nonce.
func (s *PublicTransactionPoolAPI) GetEncryptedHash(ctx context.Context, data []byte, privatefrom string, privatefor []string) ([]byte, error) {
	isPrivate := privatefor != nil
	var result []byte
	var err error
	if isPrivate {
		if len(data) > 0 {
			//Send private transaction to privacy manager
			log.Info("sending private tx", "data", fmt.Sprintf("%x", data), "privatefor", privatefor)
			result, err = private.P.Send(data, privatefrom, privatefor)
			log.Info("sent private tx", "result", fmt.Sprintf("%x", result), "privatefor", privatefor)
			if err != nil {
				return nil, err
			}
		}
	} else {
		return nil, fmt.Errorf("transaction is not private")
	}
	return result, nil
}
